...
static bool createConnection()
{
    ...
    db.setHostName("81.69.193.13");                  //域名
    db.setDatabaseName("stucms2");                      //数据库名
    db.setUserName("stucms2");                                //用户名
    db.setPassword("stucms2");                        //密码
    if (!db.open())
    { //如果无法连接
    ...
    // 创建 stu_info 表
    if (!query.exec("CREATE TABLE `stucms2`.`stu_info`  ("
    ...
    //创建stu_score表
    if (!query.exec("CREATE TABLE `stucms2`.`stu_score`  ("
                    "`stuno` int(11) NOT NULL,"
